...
# this comes from https://connect.garmin.com/gc-api/activity-service/activity/activityTypes
PARENT_TYPE_ID = {
    1: 'running',
    2: 'cycling',
    4: 'other',
...
def export_data_file(activity_id, activity_details, args, file_time, append_desc, date_time, friendly_filename):
    """
    ...
    if os.path.isfile(data_filename):
        ...
        return False, copied_files, skipped_files
                ...
                with open(data_filename, 'rb') as zip_file, zipfile.ZipFile(zip_file) as zip_obj:
                    for name in zip_obj.namelist():
                        ...
                        if len(args.workflowdirectory):
                            shutil.copyfile(os.path.join(args.directory, new_name), os.path.join(args.workflowdirectory, friendly_filename + name_ext))
                            logging.info('copy workflow file to: ' + args.workflowdirectory + '/' + friendly_filename)
                            copied_files = 1
                        if file_time:
    ...
    return True, copied_files, skipped_files
...
def process_activity_item(item, number_of_items, device_dict, type_filter, activity_type_name, event_type_name, csv_filter, args):
    ...
    global total_copied_files
    global total_skipped_files
    #total_copied_files = 0
    #total_skipped_files = 0 
    ...
    #print('friendly_filename: '+ friendly_filename)
    ...
    #searchme
    export_ok, copied_files, skipped_files = export_data_file( str(actvty['activityId']), activity_details, args, start_time_seconds, append_desc, actvty['startTimeLocal'], friendly_filename)
    if export_ok == True:
        total_copied_files += copied_files
        #print('export ok')
        #print('skipped files: ' + str(total_skipped_files))
        #print('copied files: ' + str(total_copied_files))
    else:
        total_skipped_files += skipped_files        # Write stats to CSV.
